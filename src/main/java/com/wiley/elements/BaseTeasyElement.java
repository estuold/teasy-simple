...
public abstract class BaseTeasyElement implements TeasyElement, Locatable {
    ...
    BaseTeasyElement(TeasyElementData elementData) {
        ...
        this.locator = new ElementLocatorFactory(elementData, DriverHolder.getDriver()).getLocator();
    ...
    public void init(WebElement element) {
        ...
        if (contextFinder == null) {
            contextFinder = new TeasyElementFinder(DriverHolder.getDriver(), new SearchStrategy(TIMEOUT_FOR_AGAIN_LOCATE_IN_SECONDS), this);
        }
    }
    ...
    TeasyElementFinder customContextFinder(SearchStrategy strategy) {
        return new TeasyElementFinder(DriverHolder.getDriver(), strategy, this);
    }
    ...
    private void clickForIgnoredScroll(WebDriverException ignoredOrNeedToScroll) {
        ...
        if (ignoredOrNeedToScrollMessage.contains("unknown error: no element reference returned by script")) {
            ...
            Actions actions = new Actions(DriverHolder.getDriver());
            actions
    ...
    public <X> X getScreenshotAs(OutputType<X> target) {
        if (DriverHolder.getDriver().getClass() == RemoteWebDriver.class) {
            ...
            return ((TakesScreenshot) DriverHolder.getDriver()).getScreenshotAs(target);
        }
    }
    ...
    private void increment() {
        if (repeatLocateElementCounter > MAX_NUMBER_OF_REPEAT_LOCATE_ELEMENT) {
            throw new RuntimeException("Cannot interact properly with element with locator '" + locator.getBy() + "'" + (!wrappedElement.isDisplayed() ? "Element was not displayed!" : ""));
        } else {
    ...
    private Object executeScript(String script, Object... args) {
        return ((JavascriptExecutor) DriverHolder.getDriver()).executeScript(script, args);
    }
    ...
    private FramesTransparentWebDriver getFrameTransparentDriver() {
        return (FramesTransparentWebDriver) DriverHolder.getDriver();
    }
}
